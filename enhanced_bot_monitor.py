...
class BotActivityMonitor:
    def __init__(self):
        ...
        try:
            ...
            st.success("🟢 Connected to Production Data Manager")
        except ImportError:
    ...
    def get_bot_current_activity(self):
        """Pobierz aktualną aktywność bota - priorytet dla real data"""
        try:
            # First try to get real data from production manager
            if self.production_mode:
                real_portfolio = self.get_real_portfolio_data()
                real_trading_status = self.get_real_trading_status()
                
                if real_portfolio or real_trading_status:
                    return {
                        "trading_status": real_trading_status.get("status", {"active": True}),
    ...
    def get_real_portfolio_data(self):
        """Get real portfolio data from production manager"""
        if self.production_manager and self.production_mode:
            try:
                return self.production_manager.get_portfolio_data()
            except Exception as e:
                st.error(f"Production manager error: {e}")
                return {}
        return {}
        
    def get_real_trading_status(self):
        """Get real trading status from production manager"""
        if self.production_manager and self.production_mode:
            try:
                return self.production_manager.get_trading_status()
            except Exception as e:
                st.error(f"Production manager error: {e}")
                return {}
        return {}
...
def main():
    # Ensure BotActivityMonitor is always in session state
    ...
    data_source = bot_activity.get("data_source", "unknown")
    
    if data_source == "production_manager":
        st.success("🟢 Real Data (Production Manager)")
    elif data_source == "api_fallback":
        st.info("🔵 Real Data (API)")
    else:
